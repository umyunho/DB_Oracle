...
--상대 테이블의 특정 필드값을 참조하면서 없는 값을 사용하지 않는 필드. not null
...
CREATE TABLE rentlist(
	...
	--현재 테이블의 mnum필드는 memberlist테이블의 membernum필드값을 참조합니다.
)
...
--FOREIGN KEY
--	관계형 데이터 베이스에서 테이블간에 관계를 정의하기 위해 기본키를 다른 테이블의 외래키로 복사하는 경우 기본키를 생성한다다.
-- 	- 참조 무결성 제약 옵션이 생성
